...
public class Main{
    public static void main(String[] args){
        Set<String> options = Arrays.stream(args).collect(Collectors.toSet());
        boolean fullPipeline = options.contains("--full");
        // Data preparation is enabled by default because every later phase needs it.
        boolean importData = true;
        boolean featureExtract = true;
        boolean toSPMF = fullPipeline || options.contains("--spmf");
        boolean runEclat = options.contains("--eclat");
        boolean runEclatFPgrowth = options.contains("--eclat-fpgrowth");
        boolean runFPGrowth = fullPipeline || options.contains("--fpgrowth");
        boolean runModel = fullPipeline || options.contains("--model");
        boolean resetIsTrained = fullPipeline || options.contains("--reset");
        boolean runIncremental = fullPipeline || options.contains("--incremental");
        //folder button
